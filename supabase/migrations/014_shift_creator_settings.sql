...
alter table public.workspaces
  add column if not exists default_shift_duration integer not null default 9 check (default_shift_duration between 1 and 24),
  add column if not exists ot_alert_threshold integer not null default 30 check (ot_alert_threshold between 0 and 480),
