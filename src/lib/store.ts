...
export function grantBonus(userId: string, delta: number, label: string) {
  if (!cache) return;
  cache.pointEvents.push({ id: uid(), userId, date: todayKey(), delta, label, source: "auto", category: delta >= 0 ? "bonus" : "discipline" });
  const u = userById(userId); if (u) u.points += delta;
  mutate();
}

export function addPointEvent(input: { userId: string; delta: number; label: string; reason: string; adminId: string; category: NonNullable<PointEvent["category"]> }): { ok: boolean; msg: string } {
  if (!cache) return { ok: false, msg: "Store not ready" };
