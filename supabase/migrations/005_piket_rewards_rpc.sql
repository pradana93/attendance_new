...
begin
  ...
  if found then
    ...
    values (workspace_id, auth.uid(), p_work_date, task_points, 'Piket: ' || p_task_id::text, 'auto', 'piket');
...
begin
  ...
  insert into public.point_events (workspace_id, user_id, delta, label, source, category) values (workspace_id, auth.uid(), -item_cost, 'Redeemed: ' || item_name, 'auto', 'reward');
end;
$$;
...
begin
  ...
  insert into public.point_events (
    ...
    workspace_id, p_user_id, current_date, p_delta, btrim(p_label), btrim(p_reason), 'manual', auth.uid(), p_category
  );
